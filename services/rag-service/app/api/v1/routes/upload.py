...
async def upload_document(
    ...
    doc_id = str(uuid4())
    # Save document entry
    new_doc = Document(id=doc_id, filename=file.filename, user_id=user_id, status=DocumentStatus.pending)
    session.add(new_doc)
...
async def get_document_status(document_id: str, session: Session = Depends(get_async_session)):
